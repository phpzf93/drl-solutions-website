...
import { magpiePayment, PAYMENT_METHODS } from '@/lib/magpie';
import { 
  CreditCard, 
  Trash2, 
  Plus, 
...
export default function Checkout() {
    ...
    try {
      ...
      if (response.success && response.paymentId) {
        ...
        if (response.paymentUrl) {
          ...
          // For demo purposes, simulate redirect delay
          setTimeout(() => {
            // In a real app, this would redirect to the actual payment gateway
            console.log('🌐 Simulating redirect to:', response.paymentUrl);
          ...
          // Don't clear cart yet for manual payments
          toast({
  ...
  const proceedWithManualPayment = () => {
    ...
    clearCart();
    navigate(`/order-success/${paymentDetails?.reference || 'manual'}`, {
      state: {
        paymentMethod: selectedPaymentMethod,
